...
const compileLess = async (filePath, extensions) => {
    ...
    for (const ext of extensions) {
        ...
        log.success('', `${isWindows() ? '' : '🎉 '}compile ${filePath} successfully`);
    }
};
/**
 * less
...
const lesscAction = async (dest = './', watch = false, extensions = ['css']) => {
    ...
    if (watch) {
        ...
        log.notice('', `${isWindows() ? '' : ' 🦉 '}watching less file change`);
        chokidar.watch(files).on('change', async (filePath) => {
            await compileLess(filePath, extensions);
        });
    } else {
        // 递归获取所有文件进行编译
        log.notice('', ` ${isWindows() ? '' : ' 🔍 '}getting less file path`);
        const filePaths = [];
        ...
        log.success('', `${isWindows() ? '' : '🎉 '}compile less complete,  duration: ${endTime - beginTime}ms`);
    }
};
