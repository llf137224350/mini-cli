...
const { confirm, _pathExists, getAbsolutePath, isWindows } = require('../utils/Utils');
/**
 * 批量删除空文件夹和指定扩展名文件
 */
 ...
 * @param {*} force 是否强制执行
 */
const deleteEmptyFolderOrFilesAction = async (dests = ['./'], extensions = [], force = false) => {
    ...
    for (const dest of dests) {
        ...
        log.notice('', `operating directory ${rootPath}`);
...
const execDeleteEmptyFolderOrFiles = (rootPath, extensions) => {
    ...
    log.success('', `${isWindows() ? '' : '🎉 '}delete ${extensions.map((item) => '*' + item)} and empty directory completed, include ${filePaths.length} files and ${deleteFolderCount} empty directories.    `);
};
