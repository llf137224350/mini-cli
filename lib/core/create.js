...
const createCommands = () => {
    ...
    program
        ...
        .description('create a page from a template, it is recommended to use the name of large hump for file name')
        .action((pageName) => {
            const opts = program.opts();
            const dest = opts.dest || './';
            console.log(dest, pageName);
        });
    // 根据模板创建组件
    program
        .command('component <componentName>')
        .description('create a component from a template, it is recommended to use the name of large hump for file name')
        .action((componentName) => {
            const opts = program.opts();
            const dest = opts.dest || './';
            console.log(dest, componentName);
        });
};
