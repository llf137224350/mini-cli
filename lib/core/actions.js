...
const handleConfirmPath = async (targetPath, projectName) => {
    ...
    const { type } = await inquirer.prompt({
        type: 'list',
        name: 'type',
        message: `"${projectName}" directory already exists, if you continue, it will be delete "${projectName}" directory`,
        default: 0,
        choices: [
            {
                name: 'yes',
...
const createProjectAction = async (projectName, dest = './') => {
    const targetPath = path.resolve(process.env.PWD, dest, projectName);
    const isOk = await handleConfirmPath(targetPath, projectName);
    if (!isOk) {
        return;
    }
    let spinner;
    try {
        ...
        // clone 项目
        await downloadSync(
            'direct:https://github.com/llf137224350/hello_json.git',
            targetPath,
            {
                clone: true,
