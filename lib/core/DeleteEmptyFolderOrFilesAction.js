...
const confirmAction = async (rootPath, extensions) => {
    let message = `确定删除"${rootPath}"目录下的空文件夹吗？`;
    if (extensions.length) {
        extensions = extensions.map((item) => '*' + item);
        message = `确定删除"${rootPath}"目录下的空文件夹和"${extensions}"格式的文件吗？`;
    }
 ...
 * @param {*} force 是否强制执行
 */
const deleteEmptyFolderOrFiles = async (dest = './', extensions = [], force = false) => {
    const rootPath = path.resolve(process.env.PWD, dest);
    if (!(await pathExists(rootPath))) {
        log.error('', `"${rootPath}"目录不存在，请检查！`);
        return;
    }
    let result = true;
    if (!force) {
        // 询问是否继续
        log.warn('', '即将执行删除操作，数据无价，请谨慎操作！');
...
function deleteCurrentPath(rootPath, currentDirPath) {
    ...
    if (!files.length) {
        // 空文件夹
        log.success('', `正在删除: ${currentDirPath}`);
...
const execDeleteEmptyFolderOrFiles = (rootPath, extensions) => {
    ...
    if (filePaths.length) {
        ...
        for (const currentPath of filePaths) {
            log.success('', `正在删除: ${currentPath}`);
            fs.unlinkSync(currentPath);
        }
    }
    // 删除空目录
    if (filePaths.length || dirPaths.length) {
        if (dirPaths.length === 1 && dirPaths[0] !== rootPath) {
            log.notice('', '开始删除空目录');
    ...
    log.success('', `删除wxss、js、空目录完成，删除 ${filePaths.length} 个文件，删除 ${deleteFolderCount} 个空目录。`);
};
module.exports = deleteEmptyFolderOrFiles;
