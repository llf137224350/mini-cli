...
const lesscAction = require("../action/LesscAction");
const execAction = require("../action/ExecAction");
/**
 * create指令
 */
const registerCommands = () => {
    ...
    program
        ...
        .action(async (componentName, description) => {
            const opts = program.opts();
            const dest = opts.dest || './';
            await createComponentAction(dest, componentName,description);
        });
    /**
     * 递归删除空文件夹或者指定后缀的文件
     */
    program
        ...
        .action(async () => {
            ...
            await deleteEmptyFolderOrFilesAction(dests, extensions, force);
        });
    ...
    program
        .command('lessc')
        .description('compile less as css or wxss')
        .action(async () => {
            const opts = program.opts();
            const dest = opts.dest;
            const watch = opts.watch;
            const extensions = opts.extensions;
            await lesscAction(dest, watch, extensions);
        });
    // 同步执行多条命令
    program
        .command('exec [commands...]')
        .description('execute multiple commands synchronously')
        .action(async (commands) => {
            const opts = program.opts();
            const dest = opts.dest;
            const force = opts.force;
            await execAction(commands, dest, force);
        });
    // 配置环境变量
    program
        .command('config <option> [configKeyAndValue...]')
        .description(`configure environment variables, keys:
